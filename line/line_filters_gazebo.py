    ...
    while flag:
        ...
        if contours:
            ...
            for i, cnt in enumerate(contours):
                ...
                if (max(math.fabs(bounding_cnt[3]), math.fabs(bounding_cnt[2]))/ min(math.fabs(bounding_cnt[3]), math.fabs(bounding_cnt[2]))) >= 2 \
                    and cnt_area == contours_area[0]:
                    largest = contours[i]
                    filters_flag = True
                    break
                elif 1 <= (max(math.fabs(bounding_cnt[3]), math.fabs(bounding_cnt[2]))/ min(math.fabs(bounding_cnt[3]), math.fabs(bounding_cnt[2]))) <= 1.4:
            ...
            if filters_flag == True:
                ...
                cv2.drawContours(img, contours[i], 0, (0, 255, 0), 3, cv2.LINE_AA)
                ...
                if len(largest) > 2:
                    ...
                    # print(angle)
                    cv2.putText(img, f'angle: {round(angle, 1)}', (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                    cv2.putText(img, f'center: {round(center[0], 1)}', (160, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
...
while flag:
    ...
    set_yaw_rate(yaw_rate=math.radians(angle) * -kp)
